# The Hog Printed CLI Agent Guide

This directory is a generated `thehog` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
thehog doctor --json
thehog agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
thehog which "<capability>" --json
thehog <command> --help
```
